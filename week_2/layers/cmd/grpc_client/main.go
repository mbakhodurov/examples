...
func GetSight(ctx context.Context, uuid string, client ufo_v1.UFOServiceClient) (*ufo_v1.Sighting, error) {
...
func DeleteSight(ctx context.Context, uuid string, client ufo_v1.UFOServiceClient) error {
	if uuid == "" {
		return fmt.Errorf("DeleteSight: uuid не может быть пустым")
	}
	_, err := client.Get(ctx, &ufo_v1.GetRequest{Uuid: uuid})
	if err != nil {
		return fmt.Errorf("DeleteSight: %w", err)
	}

	_, err = client.Delete(ctx, &ufo_v1.DeleteRequest{Uuid: uuid})
	if err != nil {
...
func CreatingSighting(ctx context.Context, client ufo_v1.UFOServiceClient) (string, error) {
...
func main() {
	...
	for {
		...
		switch choice {
		case "1":
			uuid, err := CreatingSighting(ctx, client)
			if err != nil {
		...
		case "2":
			...
			uuid := scanner.Text()
			sighting, err := GetSight(context.Background(), uuid, client)
			if err != nil {
		...
		case "4":
			...
			uuid := scanner.Text()
			err := DeleteSight(context.Background(), uuid, client)
			if err != nil {
