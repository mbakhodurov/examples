...
func main() {
	...
	client, err := weather_v1.NewClient(serverURL)
	if err != nil {
	...
	weatherResp, err := client.GetWeatherByCity(ctx, weather_v1.GetWeatherByCityParams{
		City: defaultCityName,
	})
	if err != nil {
		// Проверяем, что ошибка содержит информацию о статусе 404
		var errResp *weather_v1.GenericErrorStatusCode
	...
	updateRequest := &weather_v1.UpdateWeatherRequest{
		Temperature: gofakeit.Float32Range(defaultMinTemp, defaultMaxTemp),
	}

	updatedWeather, err := client.UpdateWeatherByCity(ctx, updateRequest, weather_v1.UpdateWeatherByCityParams{
		City: defaultCityName,
	})
	if err != nil {
